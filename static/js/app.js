...
async function downloadBook() {
    ...
    try {
        ...
        if (!draftResponse.success) {
            ...
            if (draftResponse.status === 409 && draftResponse.existing_draft_id) {
                ...
                if (loadExisting) {
                    ...
                    window.location.href = `/draft/${draftResponse.existing_draft_id}`;
                    return;
                } else {
        ...
        if (currentDraftId) {
            ...
            throw new Error('Draft ID not set after save');
        }
...
let indexQueueInterval = null;

// Refresh queue widget on index page
async function refreshQueueWidget() {
    try {
        ...
        if (response.ok && data.status) {
            ...
            if (readyEl) readyEl.textContent = status.ready || 0;
            if (errorEl) errorEl.textContent = status.error || 0;
            
            // Update message
            if (messageEl) {
                const total = status.queued + status.processing;
                if (total > 0) {
                    messageEl.innerHTML = `<strong>${total} task(s)</strong> in progress - Your content is being generated by AI`;
                    messageEl.style.color = '#3182ce';
                } else if (status.ready > 0) {
                    messageEl.innerHTML = `✓ All tasks completed - ${status.ready} task(s) ready`;
                    messageEl.style.color = '#38a169';
                } else {
                    messageEl.innerHTML = 'No active tasks';
                    messageEl.style.color = '#718096';
                }
            }
        }
    } catch (error) {
        console.error('Error refreshing queue widget:', error);
    }
}

// Start auto-refresh for index page queue widget
function startIndexQueueRefresh() {
    if (indexQueueInterval) {
        clearInterval(indexQueueInterval);
    }
    refreshQueueWidget(); // Initial refresh
    indexQueueInterval = setInterval(refreshQueueWidget, 5000); // 5 seconds
}

// Stop auto-refresh
function stopIndexQueueRefresh() {
    if (indexQueueInterval) {
        clearInterval(indexQueueInterval);
        indexQueueInterval = null;
    }
}
