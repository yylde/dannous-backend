...
- **Queue Management:** Real-time monitoring and control of the Ollama AI queue via header widget. Shows pending task count, active worker status, and last update time. Users can flush the queue to cancel all pending AI tasks (with confirmation). Accessible via GET `/api/queue/status` and POST `/api/queue/flush` endpoints.
