...
- **Queue Management:** Real-time monitoring and control of the Ollama AI queue via header widget. Shows pending task count, active worker status, last update time, and **detailed pending tasks list**. Each pending task displays its type (tags, description, synopsis, questions), book ID, and chapter ID (if applicable) with visual icons. Task metadata is tracked throughout the queue system from submission to completion. Users can flush the queue to cancel all pending AI tasks (with confirmation). Accessible via GET `/api/queue/status` (returns `pending_tasks` array with task metadata) and POST `/api/queue/flush` endpoints.
